...
function calculatePositionSize({ account_size, risk_pct = 2, entry_price, stop_loss, target_price }) {
  if (!account_size || account_size <= 0) return { error: 'Account size must be positive' };
  if (!entry_price || entry_price <= 0) return { error: 'Entry price must be positive' };
  if (!stop_loss || stop_loss <= 0) return { error: 'Stop loss price must be positive' };
  ...
  if (target_price && target_price > entry_price) {
...
async function preTradeCheck({ ticker, dollars_to_invest, stop_loss, userId }) {
  ...
  // Figure out the new trade's sector
  ...
  // Dollar risk if a stop loss is provided — we need current price for the math
  let dollarRisk = null;
  let riskPctOfPortfolio = null;
  if (stop_loss && Number.isFinite(stop_loss)) {
    const priceData = getPrice(ticker);
    const currentPrice = priceData?.price ?? null;
    if (currentPrice && currentPrice > stop_loss) {
      const riskPerShare = currentPrice - stop_loss;
  ...
  if (riskPctOfPortfolio != null) {
    const riskCapByTolerance = { conservative: 1, moderate: 2, aggressive: 3.5 };
    const cap = riskCapByTolerance[riskTolerance] ?? 2;
